document.addEventListener('DOMContentLoaded', () => {
  ...
  // ショートカットの読み込みと表示（保存がなければデフォルト表示）
  chrome.storage.local.get('userShortcut', ({ userShortcut }) => {
    currentShortcutDisplay.textContent = userShortcut || 'Ctrl+Shift+L';
    shortcutInput.placeholder = userShortcut || 'Ctrl+Shift+L';
  });
  ...
  document.getElementById('saveShortcutBtn').addEventListener('click', () => {
    chrome.storage.local.set({ userShortcut: shortcutInput.value }, () => {
      currentShortcutDisplay.textContent = shortcutInput.value || 'Ctrl+Shift+L';
      shortcutModal.style.display = 'none';
    });
  });
  ...
  document.getElementById('clearShortcutBtn').addEventListener('click', () => {
    chrome.storage.local.remove('userShortcut', () => {
      currentShortcutDisplay.textContent = 'Ctrl+Shift+L';
      shortcutInput.value = '';
